	...
	(function (exports, $) {
		...
		exports.TeambuilderRoom = exports.Room.extend({
			...
			renderSet: function (set, i) {
				...
				var tierBoost = (isTierShift ? getTierShiftBoost(species) : 0);
				...
				for (var j in BattleStatNames) {
					...
					// Apply Tier Shift boost (NOT HP)
					if (isTierShift && tierBoost && j !== 'hp') {
						base += tierBoost;
					}
					...
					if (j === 'hp') {
						...
						var natureMod = 1;
						if (BattleNatures[set.nature]) {
							if (BattleNatures[set.nature].plus === j) natureMod = 1.1;
							if (BattleNatures[set.nature].minus === j) natureMod = 0.9;
						}
				...
				$targets.each(function() {
					...
					var img = new Image();
					    img.onload = function() {
							...
							// background on the `.setchart` (allows the sprite to
							// overflow/extend outside the sprite cell). Only apply the
							...
							if (spriteCell.length) {
								// Check whether the GIF fits inside the sprite cell
								var cellW = spriteCell.width();
								var cellH = spriteCell.height();
								var imgW = img.naturalWidth || 0;
