...
class BattleTooltips {
	...
	getMoveppOverride(move: Move) {
		let ppO = (move as any).ppOverride || null;
		return ppO;
	}
		...
		if (move.id === 'naturepower') {
			...
			if (this.battle.gen > 5) {
				if (this.battle.hasPseudoWeather('Electric Terrain')) {
					...
					calls = 'Energy Ball';
				} else if (this.battle.hasPseudoWeather('Arcane Terrain')) {
					calls = 'Dragon Pulse';
	...
	getPPUseText(moveTrackRow: [string, number], showKnown?: boolean) {
		...
		if (moveName.charAt(0) === '*') {
			...
			const ppO = this.getMoveppOverride(move);
			maxpp = (basePP === 1 || move.noPPBoosts ? basePP : basePP * 8 / 5);
			if (ppO !== null) maxpp = ppO
	...
	getMoveBasePower(move: Move, moveType: TypeName, value: ModifiableValue, target: Pokemon | null = null) {
		...
		if (move.id === 'trumpcard') {
			let basePP = this.getMoveBasePP(move);
			const ppO = this.getMoveppOverride(move);
			if (ppO !== null) basePP = ppO
		...
		if (move.id === 'terrainpulse' && pokemon.isGrounded(serverPokemon)) {
			if (
				this.battle.hasPseudoWeather('Arcane Terrain') ||
		...
		if ((this.battle.hasPseudoWeather('Electric Terrain') && moveType === 'Electric') ||
			...
			(this.battle.hasPseudoWeather('Arcane Terrain') && moveType === 'Dragon') ||
