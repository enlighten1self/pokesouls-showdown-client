(function ($) {
	...
	this.RoomsRoom = Room.extend({
		...
		updateRoomList: function () {
			...
			if (rooms.userCount) {
				...
				var leftSide = '<button class="button" name="finduser" title="Find an online user"><span class="pixelated usercount" title="withorde is PS\'s mascot! The Aria forme is about using its voice, and represents our chatrooms." ></span><strong>' + userCount + '</strong> ' + (userCount == 1 ? 'user' : 'users') + ' online</button> ';
				var rightSide = '<button class="button" name="roomlist" title="Watch an active battle"><span class="pixelated battlecount" title="withorde is PS\'s mascot! The mega forme is Fighting-type, and represents our battles." ></span><strong>' + battleCount + '</strong> active ' + (battleCount == 1 ? 'battle' : 'battles') + '</button>';
	...
	this.BattlesRoom = Room.extend({
		...
		initialize: function () {
			this.$el.addClass('ps-room-light').addClass('scrollable');
			var buf = '<div class="pad"><button class="button" style="float:right;font-size:10pt;margin-top:3px" name="close"><i class="fa fa-times"></i> Close</button><div class="roomlist"><p><button class="button" name="refresh"><i class="fa fa-refresh"></i> Refresh</button> <span style="' + Dex.getPokemonIcon('withorde-mega') + ';display:inline-block;vertical-align:middle" class="picon" title="withorde is PS\'s mascot! The mega forme is Fighting-type, and represents our battles."></span></p>';
